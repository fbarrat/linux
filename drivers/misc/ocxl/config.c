...
#define DVSEC_VENDOR_OFFSET              0x4
#define DVSEC_ID_OFFSET                  0x8
#define DVSEC_TL_ID                      0xF000
#define   DVSEC_TL_RECV_CAP                0x18
#define   DVSEC_TL_SEND_CAP                0x20
#define   DVSEC_TL_RECV_RATE               0x30
#define   DVSEC_TL_SEND_RATE               0x50
#define DVSEC_FUNC_ID                    0xF001
#define   DVSEC_FUNC_OFF_INDEX             0x08
#define   DVSEC_FUNC_OFF_ACTAG             0x0C
#define DVSEC_AFU_INFO_ID                0xF003
#define   DVSEC_AFU_INFO_AFU_IDX           0x0A
#define   DVSEC_AFU_INFO_OFF               0x0C
#define   DVSEC_AFU_INFO_DATA              0x10
#define DVSEC_AFU_CTRL_ID                0xF004
#define   DVSEC_AFU_CTRL_AFU_IDX           0x0A
#define   DVSEC_AFU_CTRL_TERM_PASID        0x0C
#define   DVSEC_AFU_CTRL_ENABLE            0x0F
#define   DVSEC_AFU_CTRL_PASID_SUP         0x10
#define   DVSEC_AFU_CTRL_PASID_EN          0x11
#define   DVSEC_AFU_CTRL_PASID_BASE        0x14
#define   DVSEC_AFU_CTRL_ACTAG_SUP         0x18
#define   DVSEC_AFU_CTRL_ACTAG_EN          0x1A
#define   DVSEC_AFU_CTRL_ACTAG_BASE        0x1C

#define DVSEC_AFU_IDX_MASK              GENMASK(5, 0)
#define DVSEC_ACTAG_MASK                GENMASK(11, 0)
#define DVSEC_PASID_MASK                GENMASK(19, 0)
#define DVSEC_PASID_LOG_MASK            GENMASK(4, 0)

#define DVSEC_TEMPL_VERSION         0x0
#define DVSEC_TEMPL_NAME            0x4
#define DVSEC_TEMPL_AFU_VERSION     0x1C
#define DVSEC_TEMPL_MMIO_GLOBAL     0x20
#define DVSEC_TEMPL_MMIO_GLOBAL_SZ  0x28
#define DVSEC_TEMPL_MMIO_PP         0x30
#define DVSEC_TEMPL_MMIO_PP_SZ      0x38
#define DVSEC_TEMPL_MEM_SZ          0x3C
#define DVSEC_TEMPL_WWID            0x40
...
static int find_dvsec(struct pci_dev *dev, int dvsec_id)
{
	...
	while ((vsec = pci_find_next_ext_capability(dev, vsec, OCXL_EXT_CAP_ID_DVSEC))) {
		pci_read_config_word(dev, vsec + DVSEC_VENDOR_OFFSET, &vendor);
		pci_read_config_word(dev, vsec + DVSEC_ID_OFFSET, &id);
...
static int find_dvsec_afu_ctrl(struct pci_dev *dev, u8 afu_idx)
{
	...
	while ((vsec = pci_find_next_ext_capability(dev, vsec, OCXL_EXT_CAP_ID_DVSEC))) {
		pci_read_config_word(dev, vsec + DVSEC_VENDOR_OFFSET, &vendor);
		pci_read_config_word(dev, vsec + DVSEC_ID_OFFSET, &id);
		if (vendor == PCI_VENDOR_ID_IBM && id == DVSEC_AFU_CTRL_ID) {
			pci_read_config_byte(dev, vsec + DVSEC_AFU_CTRL_AFU_IDX,
					&idx);
			if (idx == afu_idx)
				return vsec;
		}
...
static int read_dvsec_tl(struct pci_dev *dev, struct ocxl_fn_config *fn)
{
	...
	pos = find_dvsec(dev, DVSEC_TL_ID);
...
static int read_dvsec_function(struct pci_dev *dev, struct ocxl_fn_config *fn)
{
	...
	pos = find_dvsec(dev, DVSEC_FUNC_ID);
	if (!pos) {
	...
	pci_read_config_dword(dev, pos + DVSEC_FUNC_OFF_INDEX, &val);
...
static int read_dvsec_afu_info(struct pci_dev *dev, struct ocxl_fn_config *fn)
{
	...
	pos = find_dvsec(dev, DVSEC_AFU_INFO_ID);
	if (!pos) {
...
int ocxl_config_read_function(struct pci_dev *dev, struct ocxl_fn_config *fn)
{
	...
	if (rc) {
		dev_err(&dev->dev,
			"Invalid Transport Layer DVSEC configuration: %d\n",
			rc);
		return -ENODEV;
	}
/* fxb also need to worry about secondary AFU */
			...
			int offset, u32 *data)
{
	...
	pci_write_config_dword(dev, pos + DVSEC_AFU_INFO_OFF, offset);
	pci_read_config_dword(dev, pos + DVSEC_AFU_INFO_OFF, &val);
	...
	pci_read_config_dword(dev, pos + DVSEC_AFU_INFO_DATA, data);
	return 0;
}
				...
				struct ocxl_fn_config *fn, int afu_idx)
{
	...
	rc = read_afu_info(dev, fn, DVSEC_TEMPL_VERSION, &val);
	if (rc)
		return rc;
			...
			struct ocxl_afu_config *afu)
{
	...
	for (i = 0; i < OCXL_TEMPL_NAME_LEN; i += 4) {
		rc = read_afu_info(dev, fn, DVSEC_TEMPL_NAME + i, &val);
		if (rc)
			return rc;
		ptr = (u32 *) &afu->name[i];
		*ptr = val;
			...
			struct ocxl_afu_config *afu)
{
	 ...
	 * Global MMIO
	 */
	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_GLOBAL, &val);
	if (rc)
		return rc;
	afu->global_mmio_bar = EXTRACT_BITS(val, 0, 2);
	afu->global_mmio_offset = EXTRACT_BITS(val, 16, 31) << 16;

	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_GLOBAL + 4, &val);
	if (rc)
		return rc;
	afu->global_mmio_offset += (u64) val << 32;

	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_GLOBAL_SZ, &val);
	if (rc)
		return rc;
	 ...
	 * Per-process MMIO
	 */
	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_PP, &val);
	if (rc)
		return rc;
	afu->pp_mmio_bar = EXTRACT_BITS(val, 0, 2);
	afu->pp_mmio_offset = EXTRACT_BITS(val, 16, 31) << 16;

	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_PP + 4, &val);
	if (rc)
		return rc;
	afu->pp_mmio_offset += (u64) val << 32;

	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MMIO_PP_SZ, &val);
	if (rc)
		return rc;
...
static int read_afu_control(struct pci_dev *dev, struct ocxl_afu_config *afu)
{
	...
	pci_read_config_byte(dev, pos + DVSEC_AFU_CTRL_PASID_SUP, &val8);
	afu->pasid_supported_log = EXTRACT_BITS(val8, 0, 4);

	pci_read_config_word(dev, pos + DVSEC_AFU_CTRL_ACTAG_SUP, &val16);
			...
			struct ocxl_afu_config *afu, u8 afu_idx)
{
	 ...
	 * access.
	 */
	WARN_ON((afu_idx & DVSEC_AFU_IDX_MASK) != afu_idx);
	afu->idx = afu_idx;
	pci_write_config_byte(dev,
			fn->dvsec_afu_info_pos + DVSEC_AFU_INFO_AFU_IDX,
			afu->idx);
	...
	rc = read_afu_info(dev, fn, DVSEC_TEMPL_AFU_VERSION, &val32);
	if (rc)
		return rc;
	...
	rc = read_afu_info(dev, fn, DVSEC_TEMPL_MEM_SZ, &val32);
	if (rc)
		return rc;
			...
			int actag_count)
{
	...
	val = actag_count & DVSEC_ACTAG_MASK;
	pci_write_config_byte(dev, pos + DVSEC_AFU_CTRL_ACTAG_EN, val);

	val = actag_base & DVSEC_ACTAG_MASK;
	pci_write_config_dword(dev, pos + DVSEC_AFU_CTRL_ACTAG_BASE, val);
}
			...
			u32 pasid_count_log)
{
	...
	val8 = pasid_count_log & DVSEC_PASID_LOG_MASK;
	pci_write_config_byte(dev, pos + DVSEC_AFU_CTRL_PASID_EN, val8);

	pci_read_config_dword(dev, pos + DVSEC_AFU_CTRL_PASID_BASE, &val32);
	val32 &= ~DVSEC_PASID_MASK;
	val32 |= pasid_base & DVSEC_PASID_MASK;
	pci_write_config_dword(dev, pos + DVSEC_AFU_CTRL_PASID_BASE, val32);
}
...
void ocxl_config_set_afu_state(struct pci_dev *dev, int pos, int enable)
{
	...
	pci_read_config_byte(dev, pos + DVSEC_AFU_CTRL_ENABLE, &val);
	if (enable)
		val |= 1;
	else
		val &= 0xFE;
	pci_write_config_byte(dev, pos + DVSEC_AFU_CTRL_ENABLE, val);
}
...
int ocxl_config_set_TL(struct pci_dev *dev, int tl_dvsec)
{
	...
	if (!recv_rate)
		return -ENOMEM;
	/*
	 * The spec defines 64 templates for messages in the Transport
	 * Layer (TL).
	 *
	...
	for (i = 0; i < PNV_OCXL_TL_RATE_BUF_SIZE; i += 4) {
		...
		pci_write_config_dword(dev, tl_dvsec + DVSEC_TL_SEND_RATE + i,
				be32_to_cpu(*ptr32));
	}
	val = recv_cap >> 32;
	pci_write_config_dword(dev, tl_dvsec + DVSEC_TL_SEND_CAP, val);
	val = recv_cap & GENMASK(31, 0);
	pci_write_config_dword(dev, tl_dvsec + DVSEC_TL_SEND_CAP + 4, val);

	/*
	 * Host -> device
	 */
	for (i = 0; i < PNV_OCXL_TL_RATE_BUF_SIZE; i += 4) {
		pci_read_config_dword(dev, tl_dvsec + DVSEC_TL_RECV_RATE + i,
				&val);
		ptr32 = (u32 *) &recv_rate[i];
		*ptr32 = cpu_to_be32(val);
	}
	pci_read_config_dword(dev, tl_dvsec + DVSEC_TL_RECV_CAP, &val);
	recv_cap = (long) val << 32;
	pci_read_config_dword(dev, tl_dvsec + DVSEC_TL_RECV_CAP + 4, &val);
	recv_cap |= val;
...
int ocxl_config_terminate_pasid(struct pci_dev *dev, int afu_control, int pasid)
{
	...
	pci_read_config_dword(dev, afu_control + DVSEC_AFU_CTRL_TERM_PASID,
			&val);
	...
	val &= ~DVSEC_PASID_MASK;
	val |= pasid & DVSEC_PASID_MASK;
	val |= BIT(20);
	pci_write_config_dword(dev, afu_control + DVSEC_AFU_CTRL_TERM_PASID,
			val);
	...
	while (EXTRACT_BIT(val, 20)) {
		...
		pci_read_config_dword(dev,
				afu_control + DVSEC_AFU_CTRL_TERM_PASID,
				&val);
	}
	return 0;
}
			...
			u32 tag_count)
{
	...
	val = (tag_first & DVSEC_ACTAG_MASK) << 16;
	val |= tag_count & DVSEC_ACTAG_MASK;
	pci_write_config_dword(dev, func_dvsec + DVSEC_FUNC_OFF_ACTAG, val);
}
